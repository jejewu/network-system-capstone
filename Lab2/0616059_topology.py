...
def config(hosts, switches, routers, DHCPServer):
    ...
    # step 1-2
    # 192.168.1.?/26 192.168.1.126(62+64) second last address
    hosts['h2'].cmd('route add default gw 192.168.1.126')
    
    # 192.168.1.?/26 192.168.1.126(62+64) second last address
    hosts['h3'].cmd('route add default gw 192.168.1.126')

    # 192.168.3.?/24 192.168.3.254 second last address
    ...
    # R2 h45->h123
    routers['r4'].cmd('route add -net 192.168.1.0/25 gw 10.0.2.1')

    routers['r2'].cmd('route add -net 192.168.1.0/25 gw 10.0.1.2')
    routers['r2'].cmd('route add -net 192.168.3.0/24 gw 10.0.0.2')
    routers['r3'].cmd('route add -net 192.168.1.0/25 gw 10.0.0.1')
